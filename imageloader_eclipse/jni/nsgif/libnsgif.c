 ...
 */
static gif_result gif_initialise_sprite(gif_animation *gif, unsigned int width,
		unsigned int height);
...
static int maskTbl[16] = { 0x0000, 0x0001, 0x0003, 0x0007, 0x000f, 0x001f,
...
gif_result gif_initialise(gif_animation *gif, size_t size, unsigned char *data) {
	...
	if (gif->buffer_position == 0) {
		...
		gif->colour_table_size = (2
				<< (gif_data[4] & GIF_COLOUR_TABLE_SIZE_MASK));
...
//		}
		 ...
		 */
		if (gif->bitmap_callbacks.bitmap_create == NULL ) {
			return GIF_DATA_ERROR;
		}

		if ((gif->frame_image = gif->bitmap_callbacks.bitmap_create(gif->width,
				gif->height)) == NULL ) {
			gif_finalise(gif);
			return GIF_INSUFFICIENT_MEMORY;
		}
	...
	if (gif->global_colour_table[0] == GIF_PROCESS_COLOURS) {
		...
		if (gif->global_colours) {
			...
			for (index = 0; index < gif->colour_table_size; index++) {
				 ...
				 */
				unsigned char *entry =
						(unsigned char *) &gif->global_colour_table[index];
...
static gif_result gif_initialise_sprite(gif_animation *gif, unsigned int width,
	...
	if ((buffer = gif->bitmap_callbacks.bitmap_create(max_width, max_height))
			== NULL )
		return GIF_INSUFFICIENT_MEMORY;
...
static gif_result gif_initialise_frame(gif_animation *gif) {
	...
	if (!first_image) {
		if (gif->frames[frame].redraw_x > offset_x) {
			gif->frames[frame].redraw_width += (gif->frames[frame].redraw_x
					- offset_x);
			gif->frames[frame].redraw_x = offset_x;
		}
		if (gif->frames[frame].redraw_y > offset_y) {
			gif->frames[frame].redraw_height += (gif->frames[frame].redraw_y
					- offset_y);
	...
	/*	if we are clearing the background then we need to redraw enough to cover the previous
	 frame too
	 */
	gif->frames[frame].redraw_required = ((gif->frames[frame].disposal_method
			== GIF_FRAME_CLEAR)
	...
	while (gif_data[0] == GIF_EXTENSION_INTRODUCER) {
		...
		case GIF_EXTENSION_APPLICATION:
			if (gif_bytes < 17)
				return GIF_INSUFFICIENT_FRAME_DATA;
			if ((gif_data[1] == 0x0b)
					&& (strncmp((const char *) gif_data + 2, "NETSCAPE2.0", 11)
							== 0) && (gif_data[13] == 0x03)
					&& (gif_data[14] == 0x01)) {
...
gif_result gif_decode_frame(gif_animation *gif, unsigned int frame) {
	...
	/*	Get the frame data
	 */
	if (gif->bitmap_callbacks.bitmap_get_buffer == NULL ) {
		return GIF_DATA_ERROR;
	}

	frame_data = (void *) gif->bitmap_callbacks.bitmap_get_buffer(
			gif->frame_image);
	if (!frame_data)
		return GIF_INSUFFICIENT_MEMORY;
	...
	if (!clear_image) {
		...
		if ((frame == 0) || (gif->decoded_frame == GIF_INVALID_FRAME)) {
			memset((char*) frame_data, GIF_TRANSPARENT_COLOUR,
			...
			if (last_undisposed_frame == -1) {
				...
				if ((return_value = gif_decode_frame(gif, last_undisposed_frame))
						!= GIF_OK)
					goto gif_decode_frame_exit;
		...
		if (gif->frames[frame].disposal_method == GIF_FRAME_CLEAR) {
			for (y = 0; y < height; y++) {
				...
				if (gif->frames[frame].transparency)
					...
					memset(frame_scanline, colour_table[gif->background_index],
							width * 4);
			}
		}
	}
...
void gif_finalise(gif_animation *gif) {
	...
	int i = 0;
	if (gif->frames) {
		for (i = 0; i < gif->decodedCount; i++) {
			if (gif->bitmap_callbacks.bitmap_destroy != NULL ) {
				gif->bitmap_callbacks.bitmap_destroy(
						gif->frames[i].frame_image);
			}
		}
		free(gif->frames);
		gif->frames = NULL;
	...
	if (gif->local_colour_table) {
		free(gif->local_colour_table);
		gif->local_colour_table = NULL;
	}
	if (gif->global_colour_table) {
