...
public abstract class AbsDrawer {
	 ...
	 * 
	 */
	private static final PorterDuffColorFilter sColorFilterForSkin = new PorterDuffColorFilter(
			IMAGE_COLORFILTER_NIGHT, Mode.MULTIPLY);
	...
	public boolean computeBounds(DrawableWrapper drawable) {
		...
		if (drawable.mScaleType == ScaleType.MATRIX) {
		...
		if (ScaleType.FIT_XY == drawable.mScaleType || fits) {
			...
			if (ScaleType.CENTER == drawable.mScaleType) {
				...
				mDrawMatrix.setRectToRect(mTempSrc, mTempDst,
						scaleTypeToScaleToFit(drawable.mScaleType));
			}
		}
		return applyBounds(drawable);
	}
	...
	public void drawContent(Canvas canvas, DrawableWrapper drawable) {
		...
		canvas.clipRect(scrollX + pLeft, scrollY + pTop, scrollX + drawable.mViewWidth
